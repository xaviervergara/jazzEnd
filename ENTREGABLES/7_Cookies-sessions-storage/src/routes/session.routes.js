...
sessionRoutes.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.redirect('/products'); //luego, redireccionamos a la pagina principal
  } catch (error) {
    console.error(error);
...
sessionRoutes.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.redirect('/products'); //el mismo navegador redirecciona automaticamente (se redirecciona desde el server)
  } catch (error) {
    res.status(400).send({ error });
  }
});
